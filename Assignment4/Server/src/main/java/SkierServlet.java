...
public class SkierServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (!isUrlValid(urlParts)) {
      ...
      LiftRideDetails liftRideDetails = new LiftRideDetails(Integer.parseInt(urlParts[7]),
          Integer.parseInt(urlParts[1]), Integer.parseInt(urlParts[3]), Integer.parseInt(urlParts[5]));
      int totalVertical = LiftRideDAO.getTotalVertical(liftRideDetails);
      //System.out.println(totalVertical);
      response.getWriter().write("The get works for Skiers! : " + urlPath + "total Vertical" + totalVertical);
    }
  }
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (!isUrlValid(urlParts)) {
      ...
      try {
        ...
        obj.addProperty("resortId", urlParts[1]);
        obj.addProperty("seasonId", urlParts[3]);
        obj.addProperty("dayId", urlParts[5]);
  ...
  private int sendSkierDataToQueue(JsonObject data) {
    try {
      ...
      channel.basicPublish(EXCHANGE_NAME, "", null, data.toString().getBytes(StandardCharsets.UTF_8));
